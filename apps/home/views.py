...
def index(request):
	...
	twoweeks_ago = week_ago - datetime.timedelta(days=7)
	data = JaGAnalyticsData.objects.filter(date__range=[week_ago,date])
	#setting dates for 1 week
	previous_data = JaGAnalyticsData.objects.filter(date__range=[twoweeks_ago,week_ago])
	revenue = data.aggregate(total=Sum('total_revenue'))
	revenue1 = previous_data.aggregate(total=Sum('total_revenue'))
